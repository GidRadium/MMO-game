...
namespace gr
{
    namespace data
    {
        ...
        void Cell::move_to(sf::Vector2f moving_to, float dist, int field_size)
        {
        ...
        void Player::update(sf::Vector2f moving_to, float dt, float speed, int field_size)
        {
            for(int i=0; i<cells.size(); i++)
                cells[i].move_to(moving_to, dt*speed, field_size);
            last_moving_to = moving_to;
        }
        void Player::update(float dt, float speed, int field_size)
        {
            for(int i=0; i<cells.size(); i++)
                cells[i].move_to(last_moving_to, dt*speed, field_size);
        }
